...
public class KafkaSink extends AbstractSink implements Configurable {
		...
		try {
			...
			for (; processedEvents < batchSize; processedEvents += 1) {
				...
				try {
					...
					if (partitionId != null) {
						record = new ProducerRecord<String, byte[]>(eventTopic, partitionId, eventKey, serializeEvent(
								event, useAvroEventFormat));
					} else {
					...
					// N.B. The producer.send() method throws all sorts of
					// RuntimeExceptions
	...
	private void translateOldProps(Context ctx) {
		...
		// Broker List
		// If there is no value we need to check and set the old param and log a
		// warning message
