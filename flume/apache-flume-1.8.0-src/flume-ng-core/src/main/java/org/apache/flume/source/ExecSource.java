 ...
 * told to restart, if the process exits for any reason, the source also exits
 * and will produce no further data. This means configurations such as
 * <tt>cat [named pipe]</tt> or <tt>tail -F [file]</tt> are going to produce the
 * desired results where as <tt>date</tt> will probably not - the former two
 * commands produce streams of data where as the latter produces a single event
 * and exits.
 * </p>
 * <p>
...
public class ExecSource extends AbstractSource implements EventDrivenSource, Configurable {
	...
	/**
	 * 读取日志的时候，如果开始字符串是2017-10-30 04:00:34这种类型标志一个event的开始或者结束
	 */
	private boolean				newLogEvent;
	...
	public void start() {
		...
		runner = new ExecRunnable(getEventHeader(), newLogEvent, listenPackages, shell, command, getChannelProcessor(),
				sourceCounter, restart, restartThrottle, logStderr, bufferCount, batchTimeout, charset);
	...
	/**
	 * 生成event header，比如计算机名，source文件名
	 * 
	 * @return
	 */
	private Map<String, String> getEventHeader() {
		...
		try {
			...
			header.put("computerName", computerName);
		} catch (Exception e) {
			logger.error("获取不到主机名");
		}

		if (StringUtils.isNotBlank(command) && command.toLowerCase().contains("tail")
				&& command.toLowerCase().contains("-f")) {
			String[] commands = command.split(" ");
			for (int i = commands.length - 1; i >= 0; i--) {
				if (StringUtils.isNotBlank(commands[i])) {
					header.put("fileName", commands[i]);
					break;
				}
			}
		}

		header.put("index", this.index);
		return header;
	}
	...
	public void configure(Context context) {
		...
		newLogEvent = context.getBoolean("newLogEvent", false);

		index = context.getString("index", "log");

		listenPackages = context.getString("listenPackages", "com.yatang,com.busi");

		if (newLogEvent) {
			logger.info("处理日志文件，以日期为event发送");
		} else {
			logger.info("OOTB处理文件，以行为event发送");
		}
	...
	private static class ExecRunnable implements Runnable {
		...
		public ExecRunnable(Map<String, String> eventHeader, boolean newLogEvent, String listenPackage, String shell,
				String command, ChannelProcessor channelProcessor, SourceCounter sourceCounter, boolean restart,
				long restartThrottle, boolean logStderr, int bufferCount, long batchTimeout, Charset charset) {

			this.newLogEvent = newLogEvent;
			this.eventHeader = eventHeader;
			this.listenPackages = listenPackage;
			this.listenPackagePatterns = new ArrayList<>();

			String[] listenPackagePatternArray = listenPackages.split(",");
			for (String listenPackagePattern : listenPackagePatternArray) {
				listenPackagePattern = listenPackagePattern.replaceAll(".", "\\.");
				listenPackagePatterns.add(".*" + listenPackagePattern + ".*");
			}
		 ...
		 */
		private String					listenPackages;
		private List<String>			listenPackagePatterns;
		...
		public void run() {
			do {
				...
				try {
					...
					StderrReader stderrReader = new StderrReader(new BufferedReader(new InputStreamReader(
							process.getErrorStream(), charset)), logStderr);
					...
					StringBuilder eventBuilder = new StringBuilder(1000);
					while ((line = reader.readLine()) != null) {
						if (isNewEvent(line, eventBuilder)) {
							String eventBody = eventBuilder.toString();
							if (isValidLog(eventBody)) {
								sourceCounter.incrementEventReceivedCount();
								synchronized (eventList) {
									String eventJson = generateJsonBody(eventHeader, eventBody);
									eventList.add(EventBuilder.withBody(eventJson.getBytes(charset)));
		...
		private boolean isValidLog(String log) {
			...
			if (log.matches(errorPattern)) {
				return true;
			}

			String exceptionPattern = ".*Exception.*";
			if (log.matches(exceptionPattern)) {
				return true;
			}

			for (String listenPackagePattern : listenPackagePatterns) {
				if (log.matches(listenPackagePattern)) {
					return true;
				}
			}
			return false;
		...
		private boolean isNewEvent(String line, StringBuilder eventBuilder) {
			if (newLogEvent) {
				return line.length() > 23 && isDateString(line.substring(0, 23)) && eventBuilder.length() != 0;
			}
			return true;
		}
		...
		private String generateJsonBody(Map<String, String> header, String eventBody) {

			Map<String, String> jsonMap = new LinkedHashMap<>();
			jsonMap.putAll(header);
			jsonMap.put("body", eventBody);
			jsonMap.putAll(header);

			String time = "";
			if (eventBody.length() > 23) {
				SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
				time = eventBody.substring(0, 23);
				time = time.replace(",", ".");
				try {
		...
		private boolean isDateString(String pStr) {
